...
public class LeaderboardFragment extends BaseFragment implements PagingScrollHelper.onPageChangeListener,
...
//        bundle.putInt(FOLLOW_USER_ID_KEY,followUserId);
    ...
    public void initialize () {
        ...
        songListAdapter = new SongListAdapter(getContext(),this);
        songListAdapter.setHeaderVisible(false);
        songListAdapter.setFooterVisible(false);

        mRvSongList.setAdapter(songListAdapter);

        horizontalPageLayoutManager = new HorizontalPageLayoutManager(4,3);
    ...
    private void initData() {
        for (int i=0;i<30;i++) {
            ...
            data.title = "十年 HD" + i;
            data.artist = "陈奕迅" + i;
            data.path = "http://www.170mv.com/kw/antiserver.kuwo.cn/anti.s?rid=MUSIC_80403&response=res&format=mp3|aac&type=convert_url&br=128kmp3&agent=iPhone&callback=getlink&jpcallback=getlink.mp3";
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        ...
        if (mainActivity != null) {
            mainActivity.play(songListAdapter.getItem(i));
        }
    }
